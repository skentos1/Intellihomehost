    ...
    <div className='text-white bg-neutral-950 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))]'>
      <div className='max-w-[75%] mx-auto h-screen flex items-center justify-center pb-8 '>
        <div className='relative w-full h-3/4 bg-cover bg-no-repeat bg-center mb-8 flex flex-wrap rounded-lg ' style={{ backgroundImage: `url(${final})` }}>
          <div className='absolute left-4 top-1/4 p-4 w-full max-w-[75%] '>
            ...
            <button onClick={scrollToInterierHome} className='relative h-12 w-[200px] overflow-hidden border border-[#6082b6] text-white shadow-2xl transition-all before:absolute before:right-0 before:top-0 before:h-12 before:w-6 before:translate-x-12 before:rotate-6 before:bg-[#6082b6] before:opacity-10 before:duration-700 hover:shadow-[#6082b6] hover:before:-translate-x-40 rounded-md font-bold my-6 py-3'>
              <span className='relative z-10'>Zistit viac</span>
            </button>
          </div>
        </div>
      </div>
